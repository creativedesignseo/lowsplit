---
description: Pre-deploy safety checklist. Use BEFORE any production deploy. Does not deploy — only validates the change is safe to deploy.
---
...
2. **Working tree is clean (or known).**
   ...
   - Confirm with the user that any uncommitted change is
     intentional.
...
4. **Config files unchanged or intentionally changed.**
   - Check the diff for hosting config, schema, and any other
...
- Read-only platform status commands (`fly status`, `vercel
  inspect`, etc.)

## Commands you may NOT run from this skill

- Any deploy command (regardless of platform)
- Any DB write against production
- `git push --force`
